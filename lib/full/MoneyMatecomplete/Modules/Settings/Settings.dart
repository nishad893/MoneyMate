...
class _SettingsPageState extends State<SettingsPage> {
  ...
  Future<void> _fetchBudgetAmount() async {
    CollectionReference budgetData = FirebaseFirestore.instance.collection('budgetData');

    DocumentSnapshot snapshot = await budgetData.doc().get();
    setState(() {
      _budgetAmount = snapshot['budgetAmount'].toString();
      _loading = false;
    });
  }
  ...
  void _onSwipe(bool value) {
    setState(() {
      _isSwitched = value;
      if (_isSwitched) {
        _budgetAmount = 'Your Budget Amount';
      } else {
        _budgetAmount = '';
      }
    });
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            SlidingSwitch(
                              background: _isSwitched ? Colors.orange : Colors.grey,
                              textOff: "",
                              textOn: "ON",
                              value: _isSwitched,
                              width: 50,
                                ...
                                setState(() {
                                  ...
                                  if (_isSwitched) {
                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (context) => BudgetModePage(),
                                      ),
                                    ).then((result) {
                                      // Assume result is the budget amount returned from BudgetModePage
                                      setState(() {
